...
| IAM role | `game-night-iac-drift` — assumed by the drift-detector CI workflow (`tofu plan -lock=false`); currently attached to AWS-managed `ReadOnlyAccess` (broad). Issue #48 tracks narrowing to per-service Describe/Get/List with secret-VALUE omissions; PR #59 attempted that and broke the workflow (tofu plan hung), reverted in PR #76. Until #48 lands, the role is effectively read-anything-in-account (S3 object reads, Cognito ListUsers, SSM parameter metadata, Secrets Manager descriptions — KMS-encrypted SecureString *values* remain unreadable since `kms:Decrypt` is not in `ReadOnlyAccess`). Plus a separate inline policy scoped to the tfstate S3 bucket and DynamoDB lock table. |
| Region | `us-east-2` |
