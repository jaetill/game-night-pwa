...
//
// Renders a small "💬 Feedback" button in the page corner; opens a modal form
// on click. Submits to POST /feedback (the feedback Lambda).
//
// The Sentry "User Feedback" widget (Tier 1) is auto-injected via
// feedbackIntegration in src/js/sentry.js; this file is the always-on,
// public-facing alternative that works even without Sentry being initialized.
//
...
function injectButton() {
  // Idempotent — don't double-inject if hot-reload re-runs the module.
  if (document.getElementById('gn-feedback-btn')) return;

  const btn = document.createElement('button');
  btn.id = 'gn-feedback-btn';
  btn.type = 'button';
...
// ── Modal form ─────────────────────────────────────────────────────────────
function openForm() {
  // Idempotent — if already open, focus it instead of opening a second one.
  const existing = document.getElementById('gn-feedback-dialog');
  if (existing) {
    existing.focus();
    return;
  }
  ...
  dialog.id = 'gn-feedback-dialog';
  dialog.innerHTML = `
    <form method="dialog" id="gn-feedback-form" style="display:flex;flex-direction:column;gap:.75rem;min-width:320px;max-width:480px;padding:.5rem;">
  ...
  dialog.querySelector('#gn-feedback-cancel').addEventListener('click', () => {
    dialog.close();
    dialog.remove();
  });

  dialog.querySelector('#gn-feedback-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    const submitBtn = dialog.querySelector('#gn-feedback-submit');
    const status = dialog.querySelector('#gn-feedback-status');
